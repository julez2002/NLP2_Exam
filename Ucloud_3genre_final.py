...
GENRE_WEIGHT = 0.1    # auxiliary genre weight — also tested at 0.5
...
def compute_genre_weights(records: list) -> torch.Tensor:
    """Inverse-frequency weights for the 3-class genre head."""
    n      = len(records)
    counts = [sum(1 for r in records if GENRE_MAP[r["genre"]] == i) for i in range(3)]
    missing = [k for k, v in GENRE_MAP.items() if counts[v] == 0]
    if missing:
        raise ValueError(f"Genre(s) absent from training data: {missing}")
    return torch.tensor([n / (3 * c) for c in counts], dtype=torch.float)
...
class MultiTaskModernBERT(nn.Module):
    ...
    Shared ModernBERT encoder with two independent classification heads:
      ...
      - genre_head  : 3-class (fiction / news / essays) ← auxiliary, training only
    """
...
def evaluate(model: nn.Module, loader: DataLoader) -> dict:
    ...
    Auxiliary task (genre head — sanity check only):
        genre_macro_f1
    ...
    metrics = {
        ...
        "genre_macro_f1": f1_score(all_genre_labels, all_genre_preds, average="macro"),
    }
...
def train(
    ...
    genre_ce  = nn.CrossEntropyLoss(weight=genre_weights.to(device))
    ...
    for epoch in range(EPOCHS):
        ...
        for step, batch in enumerate(pbar):
            ...
            g_labs = batch["genre_label"].to(device)

            with torch.autocast(device_type=device.type, dtype=torch.bfloat16, enabled=device.type == "cuda"):
                source_logits, genre_logits = model(ids, mask)
                binary_loss = source_ce(source_logits, s_labs)
                genre_loss  = genre_ce(genre_logits,   g_labs)
                loss        = (binary_loss + genre_weight * genre_loss) / ACCUM_STEPS
            ...
            pbar.set_postfix(
                ...
                loss_gen = f"{running_genre_loss  / (step + 1):.4f}",
            )
        ...
        avg_genre_loss  = running_genre_loss  / n_steps
        metrics  = evaluate(model, val_loader)

        print(
            f"\nEpoch {epoch + 1}/{EPOCHS}  |  loss={avg_loss:.4f}"
            f"  |  loss_binary={avg_binary_loss:.4f}  |  loss_genre={avg_genre_loss:.4f}"
            f"  |  bin_f1={metrics['binary_f1']:.4f}"
            f"  |  macro_f1={metrics['macro_f1']:.4f}"
            f"  |  auc_roc={metrics['auc_roc']:.4f}"
            f"  |  acc={metrics['accuracy']:.4f}"
            f"  |  genre_f1={metrics['genre_macro_f1']:.4f}"
            f"  |  src_sil={metrics['source_silhouette']:.4f}"
            f"  |  gen_sil={metrics['genre_silhouette']:.4f}"
        )
        ...
        if val_macro_f1 > best_val_macro_f1:
            ...
            with open(save_path + "_config.json", "w") as _f:
                json.dump({
                    ...
                    "num_genre_classes":  model.genre_head.out_features,
                    "pooling":            "masked_mean",
        ...
        history.append({
            ...
            "train_genre_loss":  avg_genre_loss,
...
def predict_ai_probability(text: str, model: nn.Module, tokenizer) -> float:
    ...
    Only the source_head is used; the genre_head is discarded.
    """
    model.eval()
    enc = tokenizer(
        text,
...
def run_seeds(
    ...
    for seed in SEEDS:
        ...
        best_val_f1 = train(
            ...
            binary_weights, genre_weights, tokenizer,
            save_path=save_path,
            genre_weight=genre_weight,
        )
...
def main() -> None:
    ...
    genre_weights  = compute_genre_weights(train_records)
    print(f"  Binary weights — human: {binary_weights[0]:.3f} | AI: {binary_weights[1]:.3f}")
    print(f"  Genre weights  — fiction: {genre_weights[0]:.3f} | news: {genre_weights[1]:.3f} | essays: {genre_weights[2]:.3f}")
    ...
    kwargs = dict(
        ...
        binary_weights=binary_weights, genre_weights=genre_weights,
        tokenizer=tokenizer,
    )
    ...
    # ── Save results to JSON for offline analysis / paired delta comparison ───
