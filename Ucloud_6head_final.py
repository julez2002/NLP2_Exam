...
# ── 1. Imports ─────────────────────────────────────────────────────────────────
import os
os.environ["PYTORCH_CUDA_ALLOC_CONF"] = "expandable_segments:True"  # fixed: was PYTORCH_ALLOC_CONF (silently ignored)
...
GENRE_WEIGHT = 0.1    # auxiliary genre×source weight
SEEDS        = [42, 123, 7]

_cache_tag   = f"{MODEL_NAME.replace('/', '_')}_{MAX_LEN}"
TRAIN_CACHE  = f'train_tokenized_{_cache_tag}.pt'   # auto-invalidates when MODEL_NAME or MAX_LEN changes
VAL_CACHE    = f'val_tokenized_{_cache_tag}.pt'

GENRE_MAP = {"fiction": 0, "news": 1, "essays": 2}

# genre×source cells: index = GENRE_MAP[genre] * 2 + source_label
GENRE_SOURCE_LABELS = [
...
def compute_genre_source_weights(records: list) -> torch.Tensor:
    """Inverse-frequency weights for the 6-way genre×source auxiliary head."""
    n      = len(records)
    counts = [
        sum(1 for r in records if GENRE_MAP[r["genre"]] * 2 + r["label"] == i)
        for i in range(6)
    ]
    missing = [GENRE_SOURCE_LABELS[i] for i, c in enumerate(counts) if c == 0]
    if missing:
        raise ValueError(f"Genre×source cell(s) absent from training data: {missing}")
    return torch.tensor([n / (6 * c) for c in counts], dtype=torch.float)
...
def make_collate_fn(pad_token_id: int):
    ...
    def collate_fn(batch: list) -> dict:
        ...
        source_labels, genre_source_labels, attacks = [], [], []
...
class MultiTaskModernBERT(nn.Module):
    """
    Shared ModernBERT encoder with two independent classification heads:
      - source_head      : binary  (0 = human, 1 = AI)              ← used at inference
      - genre_source_head: 6-way   (genre × source interaction)      ← auxiliary, training only
          0 fiction×human | 1 fiction×AI
          2 news×human    | 3 news×AI
          4 essays×human  | 5 essays×AI
    """
...
def evaluate(model: nn.Module, loader: DataLoader) -> dict:
    ...
    Auxiliary task (genre×source head — sanity check only):
        gs_macro_f1, gs_per_cell_f1

    Silhouette scores (pooled embeddings; cosine; capped at 2000 samples):
        source_silhouette, gs_silhouette

    Clean/attacked breakdown (symmetric metrics):
        clean_binary_f1, clean_auc_roc, clean_tpr_at_5fpr
        attacked_binary_f1, attacked_auc_roc*, attacked_tpr_at_5fpr
        (*attacked_auc_roc only present when attacked subset contains both classes.
        attacked_tpr_at_5fpr uses the full-val threshold when subset is all-AI.)

    Per-genre binary F1 (genre derived from genre_source_label // 2):
        fiction_binary_f1, news_binary_f1, essays_binary_f1
    """
    model.eval()
    ...
    # Full-val ROC — used for tpr_at_5fpr and as the threshold anchor for attacked subset
...
def train(
    ...
    genre_source_weights: torch.Tensor,
    tokenizer,
    ...
    gs_ce     = nn.CrossEntropyLoss(weight=genre_source_weights.to(device))
    ...
    for epoch in range(EPOCHS):
        ...
        for step, batch in enumerate(pbar):
            ...
            g_labs = batch["genre_source_label"].to(device)

            with torch.autocast(device_type=device.type, dtype=torch.bfloat16, enabled=device.type == "cuda"):
                source_logits, gs_logits = model(ids, mask)
                binary_loss = source_ce(source_logits, s_labs)
                gs_loss     = gs_ce(gs_logits, g_labs)
                loss        = (binary_loss + genre_weight * gs_loss) / ACCUM_STEPS
            ...
            pbar.set_postfix(
                ...
                loss_bin = f"{running_binary_loss / (step + 1):.4f}",
                loss_gs  = f"{running_gs_loss     / (step + 1):.4f}",
            )
        ...
        avg_binary_loss = running_binary_loss / n_steps
        avg_gs_loss     = running_gs_loss     / n_steps
        ...
        if val_macro_f1 > best_val_macro_f1:
            ...
            with open(save_path + "_config.json", "w") as _f:
                json.dump({
                    ...
                    "num_genre_source_classes": model.genre_source_head.out_features,
                    "pooling":                  "masked_mean",
                    "dropout":                  model.dropout.p,
                    "genre_map":                GENRE_MAP,
                    "genre_source_labels":      GENRE_SOURCE_LABELS,
        ...
        history.append({
            "epoch":             epoch + 1,
            "train_loss":        avg_loss,
            "train_binary_loss": avg_binary_loss,
            "train_gs_loss":     avg_gs_loss,
            "saved_checkpoint":  saved,
            "patience_counter":  patience_counter,
            **{k: v for k, v in metrics.items() if k != "gs_per_cell_f1"},
            "gs_per_cell_f1":    metrics["gs_per_cell_f1"],
        })
...
# ── 10. Inference ──────────────────────────────────────────────────────────────
@torch.no_grad()
def predict_ai_probability(text: str, model: nn.Module, tokenizer) -> float:
    """
    Returns the probability (0.0 – 1.0) that a text is AI-generated.
    Only the source_head is used; the genre_source_head is discarded.
    """
    model.eval()
    enc = tokenizer(
        text,
    ...
    source_logits, _ = model(enc["input_ids"], enc["attention_mask"])
    return F.softmax(source_logits, dim=-1)[0, 1].item()
...
def run_seeds(
    ...
    """Train across all SEEDS for one genre_weight. Returns {seed: metrics}."""
    results: dict         = {}
    all_run_metrics: list = []

    for seed in SEEDS:
        print(f"\n{'=' * 60}")
        print(f"[{label}]  seed={seed}  |  genre_weight={genre_weight}")
        print(f"{'=' * 60}\n")
        sys.stdout.flush()
        ...
        best_val_f1 = train(
            ...
            binary_weights, genre_source_weights, tokenizer,
            save_path=save_path,
            genre_weight=genre_weight,
        )
    ...
    print(f"\n{'=' * 60}")
    print(f"Summary [{label}]: genre_weight={genre_weight}  |  seeds={SEEDS}")
    print(f"{'=' * 60}")
    for k in all_run_metrics[0]:
        vals = [m[k] for m in all_run_metrics if k in m and isinstance(m[k], float)]
        if vals:
...
def main() -> None:
    ...
    genre_source_weights = compute_genre_source_weights(train_records)
    print(f"  Binary weights — human: {binary_weights[0]:.3f} | AI: {binary_weights[1]:.3f}")
    print("  Genre×source weights:")
    for i, (lbl, w) in enumerate(zip(GENRE_SOURCE_LABELS, genre_source_weights.tolist())):
        print(f"    [{i}] {lbl:<16} {w:.3f}")
    ...
    kwargs = dict(
        ...
        binary_weights=binary_weights, genre_source_weights=genre_source_weights,
        tokenizer=tokenizer,
    )

    # ── Run target λ ─────────────────────────────────────────────────────────
    target_results = run_seeds(GENRE_WEIGHT, f"gw{GENRE_WEIGHT}", **kwargs)

    # ── Save results to JSON for offline analysis ──────────────────────────────
    results_path = f"{SAVE_PATH}_gw{GENRE_WEIGHT}_results.json"
